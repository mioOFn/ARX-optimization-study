...
static void run_case(FILE *csv, uint32_t reps, size_t msg_bytes) {
  ...
  for (size_t i = 0; i < warmup_loops; i++) {
    Hash(512, (const BitSequence *)msg, (DataLength)msg_bytes * 8, (BitSequence *)out);
  ...
  if (csv) bench_csv_write_row_hash(csv, "blake", "blake512_hash", 0, msg_bytes, reps, ns_med, ns_sd, mib_med, mib_sd, cyc_med, cyc_sd);
...
int main(int argc, char **argv) {
  ...
  printf("BLAKE-512 full Hash() benchmark  reps=%u\n", args.reps);
