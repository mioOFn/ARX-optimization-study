...
static void run_case(FILE *csv, uint32_t reps, size_t msg_bytes) {
  ...
  for (size_t i = 0; i < warmup_blocks; i++) {
    size_t b = i % blocks;
    st.t64[0] += 1024;
    compress64(&st, (BitSequence *)(msg + b * block_bytes));
  }
  ...
  if (csv) bench_csv_write_row_hash(csv, "blake", "blake512_compress64", 0, msg_bytes, reps, ns_med, ns_sd, mib_med, mib_sd, cyc_med, cyc_sd);
...
int main(int argc, char **argv) {
  ...
  printf("BLAKE-512 compress64 benchmark (block=128 bytes)  reps=%u\n", args.reps);
