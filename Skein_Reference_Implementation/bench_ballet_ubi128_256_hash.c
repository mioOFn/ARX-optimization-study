...
static void run_case(FILE *csv, uint32_t reps, uint32_t rounds, size_t msg_bytes) {
  ...
  for (size_t i = 0; i < warmup_loops; i++) {
    BalletUBI_128_256_Hash(rounds, 256, (const u08b_t *)msg, msg_bytes, (u08b_t *)out);
  ...
  if (csv) bench_csv_write_row_hash(csv, "ballet", "ballet_ubi128_256_hash", rounds, msg_bytes, reps, ns_med, ns_sd, mib_med, mib_sd, cyc_med, cyc_sd);
...
int main(int argc, char **argv) {
  bench_args_t args;
  bench_parse_args(argc, argv, 46, 5, &args);
  bench_enable_amd64_optimizations();
  FILE *csv = bench_csv_open(argv[0], args.csv_path);
  if (csv) bench_csv_write_header_hash(csv);
  printf("Ballet-UBI-128/256 hash benchmark (hash=256 bits, rounds=%u, reps=%u)\n", args.rounds, args.reps);
  run_case(csv, args.reps, args.rounds, 1024);
  run_case(csv, args.reps, args.rounds, 64ull * 1024ull);
  run_case(csv, args.reps, args.rounds, 1024ull * 1024ull);
