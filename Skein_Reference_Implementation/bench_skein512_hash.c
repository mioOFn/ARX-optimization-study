...
static void run_case(FILE *csv, uint32_t reps, size_t msg_bytes) {
  ...
  for (size_t i = 0; i < warmup_loops; i++) {
    ...
    Skein_512_Update(&ctx, msg, msg_bytes);
  ...
  if (csv) bench_csv_write_row_hash(csv, "skein", "skein512_hash", 0, msg_bytes, reps, ns_med, ns_sd, mib_med, mib_sd, cyc_med, cyc_sd);
...
int main(int argc, char **argv) {
  ...
  printf("Skein-512 full (Init+Update+Final) benchmark  reps=%u\n", args.reps);
